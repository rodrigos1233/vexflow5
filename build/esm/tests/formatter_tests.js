...
function getTestVoices() {
    const BEAT = VexFlow.RESOLUTION / 4;
    const tickables1 = [
...
function getResolutionMultiplier(assert) {
    const [voice1, voice2] = getTestVoices();
    assert.equal(voice1.getResolutionMultiplier(), voice2.getResolutionMultiplier(), 'Voices with same smallest duration have the same resolution multiplier');
    assert.equal(Formatter.getResolutionMultiplier([voice1, voice2]), voice1.getResolutionMultiplier(), 'LCM of voice1, voice2 is voice1');
    const TIME1_4 = {
        numBeats: 1,
        beatValue: 4,
    ...
    assert.notEqual(Formatter.getResolutionMultiplier([voice3, voice4]), voice3.getResolutionMultiplier(), 'LCM of RM of voices 3 and 4 is neither RM(3) nor RM(4)');
    assert.notEqual(Formatter.getResolutionMultiplier([voice3, voice4]), voice4.getResolutionMultiplier(), 'LCM of RM of voices 3 and 4 is neither RM(3) nor RM(4)');
}
function buildTickContexts(assert) {
    const [voice1, voice2] = getTestVoices();
    const tickables1 = voice1.getTickables();
    const tickables2 = voice2.getTickables();
